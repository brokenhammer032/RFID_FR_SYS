...
all_people_faces = {}
for person_face_filename in os.listdir(f"{saved_pictures}"):
    print(person_face_filename)
    person_face = person_face_filename.split(".")[0]
    extension = person_face_filename.split(".")[-1]
    img = cv2.imread(f'{saved_pictures}/{person_face_filename}')
    cropped = mtcnn(img)
